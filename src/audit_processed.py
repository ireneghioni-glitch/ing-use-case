...
audit_processed.py
==================
Former `audit-manifest.py`.

Audit the normalized asset manifest without changing source data.

Scope: 2026-09-21 — MVP is youth-only (5 banks) with backup banks configured.
Within-bank youth/adult comparison was excluded by team decision.
...
def youth_coverage_by_bank_table(records: list[dict[str, Any]]) -> list[str]:
    """Render per-bank youth coverage for cross-bank comparison readiness."""
    ...
    all_banks = MVP_BANKS | BACKUP_TRAD | BACKUP_NEO
    for bank, audiences in sorted(coverage.items()):
        ...
        is_mvp = "yes" if bank in MVP_BANKS else "backup"
        lines.append(f"| {bank} | {youth} | {adult} | {other} | {is_mvp} |")
...
def write_report(
    ...
    lines = [
        ...
        *youth_coverage_by_bank_table(records),
    ]
...
def main() -> None:
    ...
    # Youth coverage check per MVP bank (5 minimum for cross-bank comparison).
    for bank in sorted(MVP_BANKS):
        youth_count = sum(
            1
            for record in records
            if record.get("bank") == bank
            and record.get("audience_label") == "youth_18_25"
        )
        if youth_count < MIN_YOUTH_PER_BANK:
            warnings.append(
                f"MVP bank {bank} has only {youth_count} youth records "
                f"(need {MIN_YOUTH_PER_BANK}); consider activating a backup bank "
                f"(BNP Paribas Fortis for traditional, bunq for neo)."
            )

    # Sanity check: report presence of any record outside the declared scope.
    known_banks = MVP_BANKS | BACKUP_TRAD | BACKUP_NEO
    unknown_banks = sorted(
        {
            str(record.get("bank"))
            for record in records
            if record.get("bank") not in known_banks
        }
    )
    if unknown_banks:
        warnings.append(
            "Banks found in manifest but not declared in scope: "
            + ", ".join(unknown_banks)
        )
